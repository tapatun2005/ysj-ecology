...
class SimpleSlider {
    constructor(selector, config = {}) {
        ...
        this.items = [...$selectors(this.parent, `${selector} > *`)]
    ...
    show(inx) {
        ...
        this.current = inx
        if (!inf) {
           this.list.style.transform = `translate(-${inx}00%)`
        }


        const parentW = this.parent.offsetWidth
        const itemW = this.items[0].offsetWidth
        ...
        for (let i = (this.current * diff); i < (this.current * diff) + diff; i++) {
            if (this.items[i]) {
                setTimeout(() => {
